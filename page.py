...
with open("D:/zzh/test400-519.csv", "w", newline='', encoding='utf-8-sig') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
    writer.writerow(["日期", "标题", "概括","正文"])
        ...
        if response.status_code == 200:
            ...
            for card in card_list:
                ...
                if responseA.status_code == 200:
                    ...
                    news = soupA.find('div', attrs={'id': 'c_news_detail-15858303195441036'})
                    if news:
                        abstracts = news.find('article', attrs={'class': 'e_HtmlEditor e_HtmlEditor-001 p_articles'})
                        abstract=''
                        if abstracts:
                            paras = abstracts.find_all('p')
                            for para in paras:
                                if para.find('img'):
                                    break
                                abstract+=para.text.strip()
                    else:
                        print('no news'+urlA)
                        abstract=''
                    ...
                    print(title)
                    writer.writerow([date, title, abstract,content])
                    time.sleep(2)
                else:
